...
protected:
  ...
  virtual   double getCost(unsigned int * G1_to_G2,unsigned int * G2_to_G1, int n, int m);
...
public:
  ...
  void setCurrentMatrix(unsigned int * G1_to_G2, unsigned int * G2_to_G1, int n, int m ){//N and m are graph sizes
    if(this->Xk == NULL)
