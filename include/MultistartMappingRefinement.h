                ...
                unsigned int * G1_to_G2, unsigned int * G2_to_G1 )
{
  ...
  // Memoy cleaning
  for (std::list<unsigned int*>::iterator it = mappings.begin(); it!=mappings.end(); it++)
    delete[] *it;
}
                ...
                std::list<unsigned int*>& mappings )
{
  ...
  // Multithread
    ...
    double* arrayCosts = new double[mappings.size()];
    unsigned int* arrayLocal_G1_to_G2 = new unsigned int[n * mappings.size()];
    unsigned int* arrayLocal_G2_to_G1 = NULL;
    if (G2_to_G1 != NULL)
      arrayLocal_G2_to_G1 = new unsigned int[m * mappings.size()];

    unsigned int i=0; for (it=mappings.begin(); it!=mappings.end(); it++){
      arrayMappings[i] = *it;
      i++;
    }
    ...
    for (unsigned int tid=0; tid<mappings.size(); tid++){
      unsigned int* lsapMapping = arrayMappings[tid];
      unsigned int* local_G1_to_G2 = &(arrayLocal_G1_to_G2[tid*n]);

      unsigned int* local_G2_to_G1 = NULL;
      if (G2_to_G1 != NULL)
        local_G2_to_G1 = &(arrayLocal_G2_to_G1[tid*m]);

  // Sequential
  #else
    unsigned int* local_G1_to_G2 = new unsigned int[n];
    unsigned int* local_G2_to_G1 = NULL;
    if (G2_to_G1 != NULL)
      local_G2_to_G1 = new unsigned int[m];
    ...
    if (local_G2_to_G1 != NULL){
      for (unsigned int j=0; j<m; j++) local_G2_to_G1[j] = -1;
      for (unsigned int i=0; i<n; i++)
  ...
  // Multithread : Reduction
    ...
    for (unsigned int i=0; i<n; i++) G1_to_G2[i] = arrayLocal_G1_to_G2[i_optim*n + i];

    if (G2_to_G1 != NULL)
      for (unsigned int j=0; j<m; j++) G2_to_G1[j] = arrayLocal_G2_to_G1[i_optim*m + j];
...
template<class NodeAttribute, class EdgeAttribute>
const std::list<unsigned int*>& MultistartMappingRefinement<NodeAttribute, EdgeAttribute>::
getBetterMappings( MappingRefinement<NodeAttribute, EdgeAttribute> * algorithm,
                   Graph<NodeAttribute,EdgeAttribute> * g1,
                   Graph<NodeAttribute,EdgeAttribute> * g2 )
{
