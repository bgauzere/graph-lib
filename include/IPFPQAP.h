 ...
 *
 * Implements IPFP algorithm for graph matching 
 * Ref : Leordeanu, M.,
 * Hebert, M., & Sukthankar, R. (2009). An integer projected fixed
 * point method for graph matching and map inference. In Advances in
 * neural information processing systems (pp. 1114-1122).
 */

#ifndef __IPFPQUAP_H__
#define __IPFPQUAP_H__
...
protected:
  ...
  virtual double linearCost(double * CostMatrix, unsigned int * G1_to_G2, int n, int m);
  // n is nb rows of matrices, m is nb columns
  ...
  virtual double getCost(unsigned int * G1_to_G2, int n, int m);
  virtual double getCost(double * Matrix , int n, int m);
  virtual double getAlpha();
  virtual double getBeta();

  virtual double * mappingsToMatrix(unsigned int * G1_to_G2, int n, int m, double * Matrix);
...
public:
  ...
  virtual void getBetterMapping( Graph<NodeAttribute,EdgeAttribute> * g1,
				 ...
				 unsigned int* G2_to_G1=NULL, bool fromInit=true);
               ...
               double * quadraticTerm )
{
  ...
  for(unsigned int j = 0; (int)(j) < n; j++){
    for(unsigned int l = 0; (int)(l) < m;l++){
			  ...
			  unsigned int* G1_to_G2, double * X0)
{
  ...
  memset(G2_to_G1,0,sizeof(unsigned int)*this->_m);
  lsape::lsapeSolver<double>(this->Xk,this->_n, this->_m, G1_to_G2, G2_to_G1, u,v,lsape::ECBP,1);
               ...
               Graph<NodeAttribute,EdgeAttribute> * g2 )
{
  ...
  double *u = new double[this->_n];
  double *v = new double[this->_m];
  unsigned int * G1_to_G2 = new unsigned int[this->_n];
  unsigned int * G2_to_G1 = new unsigned int[this->_m];
  ...
  while((k < this->maxIter) && flag_continue){ //TODO : fixer un epsilon, param ?
    ...
    lsape::lsapeSolver<double>(linearSubProblem,this->_n, this->_m, G1_to_G2, G2_to_G1, u,v,lsape::ECBP,1);
...
template<class NodeAttribute, class EdgeAttribute>
double IPFPQAP<NodeAttribute, EdgeAttribute>::
linearCost(double * CostMatrix, unsigned int * G1_to_G2, int n, int m){
  double sum = 0.0;
  for(int i=0;i<n;i++)
...
template<class NodeAttribute, class EdgeAttribute>
double IPFPQAP<NodeAttribute, EdgeAttribute>::
getCost(unsigned int * G1_to_G2, int n, int m){
...
template<class NodeAttribute, class EdgeAttribute>
double * IPFPQAP<NodeAttribute, EdgeAttribute>::
mappingsToMatrix(unsigned int * G1_to_G2, int n, int m, double * Matrix){
