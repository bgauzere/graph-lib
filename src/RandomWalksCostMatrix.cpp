/*
 * @file RandomWalksCostMatrix.cpp
...
int * RandomWalksCostMatrix::labeledKron(int *m1, int nb_rows_m1,int nb_cols_m1,
...
void RandomWalksCostMatrix::computeCostMatrix(Graph<int,int> * g1,
					      Graph<int,int> * g2){
  ...
  Map<MatrixXd> matrixC(this->_C,n+1,m+1);
			
  RowVectorXd C_sub = (((_k-ILxt.array())*_cf->cns()+_k*_cf->ces()).array()*S.array() +
  		       ((1+_k-ILxt.array())*_cf->cns()+_k*_cf->ces()).array()*Ri.array().min(Rj.array()) +
  		       ((1+_k-ILxt.array())*_cf->cnd()+_k*_cf->ced()).array() *(Ri-Rj).array().abs()).cast<double>();
  
  matrixC.block(0,0,n,m) = Map<MatrixXd>(C_sub.data(),m,n).transpose();
  
  RowVectorXd Cie = (((_k+1)*_cf->cnd()+_k*_cf->ced())* (H1.cast<double>().colwise().sum().array())).cast<double>();
  RowVectorXd Cej = ((_k+1)*_cf->cnd()+_k*_cf->ced())*H2.cast<double>().colwise().sum().array().cast<double>();
...
MatrixXi RandomWalksCostMatrix::histoLab(int nbLab,  RowVectorXi IL, MatrixXi W){
